...
if (isset($_POST['submit_donation'])) {
    ...
    if ($goal_amount <= 0) {
        $errors[] = "- Goal amount must be greater than RM 0.";
    }
    if ($start_date < $today) {
    ...
    $sql = "INSERT INTO programs (title, description, type, image_path, goal_amount, current_amount, start_date, end_date, start_time, end_time, support_file, created_by) 
            VALUES ('$title', '$description', '$type', '$image_path', '$goal_amount', 0.00, '$start_date', '$end_date', '$start_time', '$end_time', '$support_file_path', '$current_staff_id')";

    if ($conn->query($sql)) {
        echo "<script>
            alert('Success! Your donation campaign has been launched.');
...
<head>
    ...
    <title>Create Donation | CharityCare+</title>
...
<body>
    ...
    <div class="title-bar">
        <div style="display: flex; align-items: center; gap: 20px;">
            ...
            <h5>Create New Donation Campaign</h5>
        </div>
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-8 mb-4">
                        ...
                        <input type="text" name="title" class="form-control" placeholder="Enter the campaign title" required>
                    </div>
                    
                    <div class="col-md-4 mb-4">
                        <label class="form-label">Goal Amount (RM)</label>
                        <input type="number" step="0.01" name="goal_amount" class="form-control" placeholder="RM 0.00" required>
                    </div>

                    <div class="col-12 mb-4">
                        <label class="form-label">Campaign Description</label>
                        <textarea name="description" class="form-control" placeholder="Detail the purpose of these donations and how they will be used..." required></textarea>
                    </div>
                ...
                <button type="submit" name="submit_donation" class="btn-submit">Launch Campaign</button>
            </form>
        </div>
    </main>
